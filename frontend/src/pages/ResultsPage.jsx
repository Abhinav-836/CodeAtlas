...
const ResultsPage = () => {
  ...
  const handleDownload = () => {
    ...
    const url = reportsAPI.downloadReportUrl(result.report_id, "json");
    window.open(url, "_blank", "noopener");
  };

  // ---- Loading / in-progress screen ----
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-6">
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="flex gap-3">
              <div
                className={`px-4 py-2 rounded-lg text-white text-center ${getRiskColor(
                  result?.overall_risk_level
                )}`}
              >
                <div className="text-xs opacity-90">Risk Level</div>
                <div className="font-bold text-lg">
                  {result?.overall_risk_level?.toUpperCase() || "UNKNOWN"}
                </div>
              </div>
              <div className="bg-gray-800 dark:bg-gray-700 px-4 py-2 rounded-lg text-white text-center">
                ...
                <div className="font-bold text-lg">
                  {result?.overall_risk_score || 0}/100
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 flex flex-wrap gap-4 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold">
                {result?.performance?.analysis_duration_seconds?.toFixed(2)}s
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-gray-600 dark:text-gray-400">
          ...
          {[
            {
              ...
              value: result?.summary?.total_files || 0,
              label: "Total Files",
            },
            {
              icon: "📊",
              ...
              value: (result?.metrics?.total_lines || 0).toLocaleString(),
              label: "Lines of Code",
            },
            {
              icon: "🌐",
              ...
              value: result?.languages?.language_count || 0,
              label: "Languages",
            },
            {
              icon: "🔒",
              ...
              value: result?.security?.vulnerabilities_found || 0,
              label: "Issues Found",
            },
          ].map((s) => (
            <Card key={s.label} className="p-4 hover:shadow-lg transition-shadow">
              <div className="flex items-center gap-3">
                <div
                  className={`text-3xl ${s.color} w-12 h-12 rounded-lg flex items-center justify-center`}
                >
                  {s.icon}
                </div>
                <div>
          ...
          <Card className="p-6 mb-6 border-2 border-blue-400 dark:border-blue-600">
            ...
            <div
              className={`prose dark:prose-invert max-w-none transition-all ${
                !aiExpanded ? "max-h-96 overflow-hidden relative" : ""
              }`}
            >
              ...
              <div>
                ...
                <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 space-y-3">
                  {[
                    ...
                    [
                      "Total Lines",
                      result?.metrics?.total_lines?.toLocaleString(),
                    ],
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  ["low", "text-blue-500", "border-l-blue-500"],
                ].map(([level, textColor, borderColor]) => (
                  <div
                    key={level}
                    className={`text-center p-4 border-l-4 ${borderColor} bg-gray-50 dark:bg-gray-900 rounded`}
                  >
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  ["total_imports", "Imports", "text-teal-600"],
                ].map(([field, label, color]) => (
                  <div
                    key={field}
                    className="text-center p-4 bg-gray-50 dark:bg-gray-900 rounded"
                  >
                    <div className={`text-2xl font-bold ${color}`}>
                      {typeof result.python_analysis[field] === "number"
                        ? result.python_analysis[field].toFixed?.(2) ??
                          result.python_analysis[field]
                        : result.python_analysis[field]}
                    </div>
                    <div className="text-sm text-gray-600 dark:text-gray-400">
                      {label}
              ...
              {result.python_analysis.most_complex_files?.length > 0 && (
                <div>
                  ...
                  <div className="space-y-3">
                    {result.python_analysis.most_complex_files
                      .slice(0, 5)
                      .map((file, idx) => (
                        <div
                          key={idx}
          ...
          {activeTab === "languages" &&
            result?.languages?.detected_languages && (
              <div className="space-y-4">
                ...
                {Object.entries(result.languages.detected_languages).map(
                  ([lang, data]) => (
                    <div
                      key={lang}
        ...
        <div className="flex flex-wrap justify-center gap-4 mt-8">
          ...
          >
            <span className="flex items-center gap-2">
              <span>⬇️</span>
              <span>
                {canDownload ? "Download JSON" : "Download unavailable"}
              </span>
            </span>
          </Button>
